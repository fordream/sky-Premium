package org.com.atmarkcafe.sky.dialog;

import java.util.ArrayList;
import java.util.List;

import com.SkyPremiumLtd.SkyPremium.R;
import org.com.atmarkcafe.view.PhoneCodeListItemView;
import org.json.JSONObject;

import z.lib.base.BaseAdapter;
import z.lib.base.BaseAdialog;
import z.lib.base.BaseItem;
import z.lib.base.BaseView;
import z.lib.base.LogUtils;
import z.lib.base.SkyAnimationUtils.AnimationAction;
import android.content.Context;
import android.os.Bundle;
import android.view.View;
import android.view.WindowManager;
import android.widget.AdapterView;
import android.widget.AdapterView.OnItemClickListener;
import android.widget.ListView;

public class PostalCodeDialog extends BaseAdialog implements android.view.View.OnClickListener {
	protected static final String TAG = "DemoDialog";
	ListView postalcodelistview;
	private BaseAdapter baseAdapter;
	public PostalCodeDialog(Context context, OnClickListener clickListener) {
		super(context, clickListener);
		getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
	}

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		initHeader();
		postalcodelistview = (ListView) findViewById(R.id.listview_phone_code);
		findViewById(R.id.header_btn_left).setOnClickListener(this);
		openPopActivity(findViewById(R.id.dialog_postalcode));
        String[] recourseList=getActivity().getResources().getStringArray(R.array.CountryCodesNew);
		
        List<BaseItem> baseItems = new ArrayList<BaseItem>();
		baseItems.clear();
		for(int i = 0; i < recourseList.length; i++){
			String[] temp =recourseList[i].split(",");
			JSONObject itemObj = new JSONObject();
		    try
		    {
		    	itemObj.put("postal_code", temp[0].trim());
		        itemObj.put("counttry_name", temp[1].trim());
		        baseItems.add(new BaseItem(itemObj));
		    }catch(Exception e){
		    	LogUtils.e(TAG, "e2" +e.getMessage());
		    }
		    
		}
        
        baseAdapter = new BaseAdapter(getActivity(), new ArrayList<Object>()) {
			@Override
			public BaseView getView(Context context, Object data) {
				final PhoneCodeListItemView phonecodeListItemView = new PhoneCodeListItemView(context);
				return phonecodeListItemView;
			}
		};
		baseAdapter.clear();
		baseAdapter.addAllItems(baseItems);
		baseAdapter.notifyDataSetChanged();
		postalcodelistview.setOnItemClickListener(new OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				BaseItem baseItem = (BaseItem) parent.getItemAtPosition(position);
				ProfileUpdateDialog.country_phone_code = baseItem.getString("postal_code");
				ProfileUpdateDialog.setTelCode(baseItem.getString("postal_code"));
				close(true);
				LogUtils.e(TAG,ProfileUpdateDialog.country_phone_code);
				
			}
		});
		postalcodelistview.setAdapter(baseAdapter);
	}

	private void initHeader() {
	}

	private void close(final boolean isOnClick) {
		closePopActivity(getContext(), findViewById(R.id.dialog_postalcode), new AnimationAction() {

			@Override
			public void onAnimationEnd() {
				dismiss();
				if (isOnClick) {
					clickListener.onClick(null, 0);
				}
			}
		});
	}

	@Override
	public int getLayout() {
		return R.layout.user_dialog_counttry_code;
	}

	@Override
	public void onClick(View v) {
		if(v.getId() == R.id.header_btn_left){
			close(true);
		}
	}

	private Context getActivity() {
		return getContext();
	}

}