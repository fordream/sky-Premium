...
public class notifiServiceMail extends Service {
...
//	private ImageView chatHead;
	...
	public void onCreate() {
	    ...
	    layout = (LinearLayout) inflater.inflate (R.layout.main_notif_mail, null);
		...
		layout.setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					try {
						LogUtils.i("XXXXXXXXXX", "ACTION-UP");
				...
				case MotionEvent.ACTION_MOVE:
					try {
						...
						if( ( Math.abs(initialX - params.x) > delta_change) || ( Math.abs(initialY - params.y) > delta_change) ){ //
							...
							LogUtils.i("XXXXXXXXXX", "DELTA_CHANGE = " + delta_change);
						}
...
//        }
    	...
    	SkyMainActivity.FLAG_CHECK_OPEN_MAIL_BOX_FROM_CHAT_HEADER = true;
    	launcherApp.addFlags(Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
//    	SharedPreferences shareData = getApplicationContext().getSharedPreferences(SkyUtils.shareKey, Context.MODE_PRIVATE);
//    	shareData.edit().putBoolean("OPEN_MAIL", true).commit();
    	startActivity(launcherApp);
		LogUtils.i("NotifyService", "OPEN APP FROM SERVICE-MailService");
	}
	...
	protected void onStop() {
    	...
    	stopService(new Intent(myService, notifiServiceMail.class));
	}
    ...
    public int onStartCommand (Intent intent, int flags, int startId) {
		LogUtils.i("AAAAAAA", "onStartCommand======BADGE_EMAIL");
		KEY_NUMBER = "";
    	try {
    	...
    	try {
			...
			LogUtils.i("AAAAAAA", "KEY_NUMBER---" + KEY_NUMBER + ":KEY_TYPE----" + KEY_TYPE);
    		TextView number = (TextView) layout.findViewById(R.id.number_value_mail);
    		if(!"".equals(KEY_NUMBER) && !"0".equals(KEY_NUMBER)){
